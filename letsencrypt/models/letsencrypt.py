...
class Letsencrypt(models.AbstractModel):
    ...
    @api.model
    def call_cmdline(self, cmdline, loglevel=logging.INFO, raise_on_result=True):
    ...
    def generate_account_key(self):
        ...
        if not os.path.isfile(account_key):
            ...
            self.call_cmdline(
                ["openssl", "genrsa", "-out", account_key, str(DEFAULT_KEY_LENGTH),]
            )
    ...
    def generate_domain_key(self, domain):
        ...
        if not os.path.isfile(domain_key):
            ...
            self.call_cmdline(
                ["openssl", "genrsa", "-out", domain_key, str(DEFAULT_KEY_LENGTH),]
            )
        return domain_key
    ...
    def generate_csr(self, domain):
        ...
        with tempfile.NamedTemporaryFile() as cfg:
            ...
            cmdline = [
                ...
                "req",
                "-new",
                parameter_model.get_param("letsencrypt.openssl.digest", "-sha256"),
                "-key",
                self.generate_domain_key(domain),
                "-subj",
                "/CN=%s" % domain,
                "-config",
                cfg.name,
    ...
    def cron(self):
        domain = urlparse(
            self.env["ir.config_parameter"].get_param("web.base.url", "localhost")
        ).netloc
        ...
        with open(os.path.join(get_data_dir(), "%s.crt" % domain), "w") as crt:
            crt.write(crt_text)
            chain_cert = urlopen(
        ...
        if reload_cmd:
            ...
            _logger.info(
                "no command defined for reloading webserver, please "
